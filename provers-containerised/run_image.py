...
def get_rlr_args(args):
    ...
    if args.starexec:
        paths_part = (
            f" --add-eof --watcher-data /export/starexec/{sandbox}/output/watcher.out"
...
def get_env_vars(args):
    sandbox = "sandbox" if args.starexec == 1 else "sandbox2"
    input_file = f"/export/starexec/{sandbox}/benchmark/theBenchmark.p" if args.starexec else "/artifacts/CWD/problemfile"
...
def main():
    ...
    parser.add_argument("-C", "--cpu-limit", default=0, type=int, help="CPU time limit in seconds, default=none")
    parser.add_argument("-W", "--wall-clock-limit", default=0, type=int, help="Wall clock time limit in seconds, default=none")
    parser.add_argument("-M", "--memory-limit", default=0, type=int, help="Memory limit in MiB, default=none")
    parser.add_argument("-I", "--intent", default="THM", choices=["THM", "SAT"], help="Intention (THM, SAT, etc), default=THM")
    parser.add_argument("--dry-run", action="store_true", help="dry run")
    parser.add_argument("-s", "--starexec", type=int, default=0, help="""This is for using run_image.py from inside a containerized starexec instance. 
    ...
    volumes = f"-v volExport:/export" if args.starexec else "-v .:/artifacts/CWD"
    remote = "--remote --connection host-machine-podman-connection" if args.starexec else ""
