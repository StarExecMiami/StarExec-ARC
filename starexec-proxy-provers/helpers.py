...
def findImageOnline(imageName):
    """
    Check if the image exists in an online registry such as docker.io.
    ...
    if registryURL in [
        ...
        if digest:
            # Query by digest is more complex, and requires Docker
            # Registry API. For simplicity, we'll not handle digest
            # in this example.
            return False

        url = (
            f"https://registry.hub.docker.com/v2/repositories/"
            f"{namespace}/{repo_name}/tags/{tag}/"
        )
        ...
        try:
            ...
            print(f"Error checking image online: {e}")
            return False
...
def verifyImageExists(imageName):
    ...
    if findImageOnline(imageName):
        print(
                f"Found '{imageName}' online. "
                f"This is the preferred way to do things."
            )
    elif findImageLocal(imageName):
        print(
            (
                f"Found '{imageName}' locally. However, in the cloud setting, "
                f"be aware that EKS must be able to find it and that having "
                f"it locally on your development machine is not enough."
            )
        )
    else:
        print(
